...
class Message(IronMQRouter):
    ...
    def __init__(self,
                 body=None, values=None, queue=None, client=None,
                 **kwargs):
        if queue is not None:
            if isinstance(queue, Queue):
                ...
                raise ValueError("Unsupported type passed for queue." +
                                 " Please use a Queue object or a string " +
                                 "containing the queue's name.")
        else:
        ...
        attrs = [x for x in vars(self.__class__).keys()
                 if not (x.startswith("_") or
                         hasattr(vars(self.__class__)[x], '__call__'))]
    ...
    def delete(self):
        if self._queue is None or (self._queue.name is None or
                                   self._queue.name == ""):
            raise ValueError("Cannot delete a message " +
                             "until its queue's name is set.")
        if self.id is None:
            raise ValueError("Cannot delete a message if its ID is not set.")

        endpoint = super(Message, self).to_path(
            'queues/%s/messages/%s' % (self._queue.name, self.id))
        try:
    ...
    def touch(self):
        if self._queue is None or (self._queue.name is None or
                                   self._queue.name == ""):
            raise ValueError("Cannot touch a message" +
                             " if its queue's name is not set.")
        if self.id is None:
        ...
        path = super(Message, self).to_path(
            'queues/%s/messages/%s/touch' % (self._queue.name, self.id))
    ...
    def release(self, delay=None):
        if self._queue is None or (self._queue.name is None or
                                   self._queue.name == ""):
            raise ValueError("Cannot release a message " +
                             "if its queue's name is not set.")
        if self.id is None:
        ...
        headers = {"Content-Type": "application/json"}
        path = super(Message, self).to_path(
            'queues/%s/messages/%s/release' % (self._queue.name, self.id))
    ...
    def get_push_status(self):
        if self._queue is None or (self._queue.name is None or
                                   self._queue.name == ""):
            raise ValueError("Cannot get the push status of a message " +
                             "if its queue's name is not set.")
        if self.id is None:
            raise ValueError("Cannot get the push status of a message " +
                             "if its ID is not set.")
    ...
    def raw(self):
        return super(Message, self).parse_response(
            {'id': self.id,
             'body': self.body,
...
class Queue(IronMQRouter):
    ...
    __ignore = ['id']
    __aliases = {'id': '_id', 'size': '_size',
                 'total_messages': '_total_messages',
                 'project_id': '_project_id', 'retries': '_retries',
                 'push_type': '_push_type', 'retries_delay': '_retries_delay',
    ...
    def info(self, instantiate=False):
        if self.name is None or self.name == "":
            raise ValueError("Cannot get queue information " +
                             "until the queue's name attribute is set.")

        try:
            resp = self._client.get(
                super(Queue, self).to_path('queues/%s' % self.name))
            ...
            if resp_code == requests.codes.not_found and not instantiate:
                return super(Queue, self).parse_response({'name': self.name,
                                                          'size': 0,
                                                          'total_messages': 0,
                                                          'subscribers': []})
            else:
    ...
    def clear(self):
        if self.name is None or self.name == "":
            raise ValueError("Cannot clear a queue " +
                             "until its name attribute is set.")

        resp = self._client.post(
            super(Queue, self).to_path('queues/%s/clear' % self.name))
    ...
    def post(self, *messages, **kwargs):
        ...
        resp = self._client.post(
            super(Queue, self).to_path('queues/%s/messages' % self.name),
    ...
    def peek(self, count=None):
        if self.name is None or self.name == "":
            raise ValueError("Cannot peek at a queue's messages " +
                             "until its name attribute is set.")

        path = super(Queue, self).to_path(
            'queues/%s/messages/peek' % self.name, n=count)
        resp = self._client.get(path)

        return self._instantiate_messages(resp['body']['messages'], count)

    def update(self, push_type=None, subscribers=None,
               retries=None, retries_delay=None, ignore_empty=True):
        queue_data = super(Queue, self).remove_empty(
            {'push_type': push_type,
             'retries': retries,
             'retries_delay': retries_delay})

        if subscribers is not None:
            queue_data['subscribers'] = self._parse_subscribers(
                subscribers,
    ...
    def subscribe(self, subscribers, ignore_empty=False):
        if self.name is None or self.name == "":
            raise ValueError("Cannot subscribe to a queue " +
                             "until its name attribute is set.")

        subscriptions = self._parse_subscribers(subscribers,
                                                ignore_empty=ignore_empty)
        data = json.dumps({'subscribers': subscriptions})
        headers = {'Content-Type': 'application/json'}

        resp = self._client.post(
            super(Queue, self).to_path('queues/%s/subscribers' % self.name),
            body=data, headers=headers)
    ...
    def unsubscribe(self, subscribers, ignore_empty=False):
        if self.name is None or self.name == "":
            raise ValueError("Cannot unsubscribe from a queue " +
                             "until its name attribute is set.")
    ...
    def _parse_subscribers(self, subscribers, ignore_empty=True):
        ...
        for subscriber in subscribers:
            if isinstance(subscriber, Subscription):
                ...
                raise ValueError('subscribers must be string or ' +
...
class Subscription(IronMQRouter):
    ...
    def __init__(self, url=None, values={},
                 message=None, queue=None, client=None, **kwargs):
        if values is None:
            values = {}
        if url is not None:
            values['url'] = url
        ...
        if message is not None:
            if isinstance(message, Message):
                ...
                raise ValueError("Unsupported type passed as a message. " +
                                 "Please pass a Message object or " +
        ...
        if queue is not None:
            if isinstance(queue, Queue):
                ...
                raise ValueError("Unsupported type passed as a queue. " +
                                 "Please pass a Queue object or " +
        ...
        if self._client is None:
            raise ValueError("Cannot initialize " +
                             "a subscription without a client.")
    ...
    def acknowledge(self):
        if self._id is None:
            raise ValueError("Cannot acknowledge a message " +
                             "if the subscription's ID is not set.")
        if self._queue is None or (self._queue.name is None or
                                   self._queue.name == ""):
            raise ValueError("Cannot acknowledge a message from " +
                             "a subscription whose queue's name is not set.")
        if self._message is None or self._message.id is None:
            raise ValueError("Cannot acknowledge a message " +
                             "if its ID is not set.")

        endpoint = super(Subscription, self).to_path(
            'queues/%s/messages/%s/subscribers/%s' % (self._queue.name,
                                                      self._message.id,
                                                      self._id))
        try:
            resp = self._client.delete(endpoint)
...
class IronMQ(IronMQRouter):
    ...
    def queues(self, per_page=None, page=None, instantiate=False):
        resp = self.client.get(
            super(IronMQ, self).to_path('queues',
